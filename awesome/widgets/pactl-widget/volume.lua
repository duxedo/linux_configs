...
local widget_types = {
    icon_and_text = require("widgets.pactl-widget.widgets.icon-and-text-widget"),
    icon = require("widgets.pactl-widget.widgets.icon-widget"),
    arc = require("widgets.pactl-widget.widgets.arc-widget"),
    horizontal_bar = require("widgets.pactl-widget.widgets.horizontal-bar-widget"),
    vertical_bar = require("widgets.pactl-widget.widgets.vertical-bar-widget")
}
local volume = {}
...
local function worker(user_args)
    ...
    local function update_graphic(widget, do_notify)
        local vol = pactl.get_volume(device)
        if vol ~= nil then
            widget:set_volume_level(vol)
        end
        local icon = not vol or vol < 33 and "audio-volume-low" or (vol >= 33 and vol < 66) and "audio-volume-medium" or "audio-volume-high"
    ...
    function volume:inc(s, do_notify)
        pactl.volume_increase(device, s or step)
        update_graphic(volume.widget, do_notify)
    end

    function volume:dec(s, do_notify)
        pactl.volume_decrease(device, s or step)
        update_graphic(volume.widget, do_notify)
    end

    function volume:toggle(do_notify)
